...
function sub(string $text, array $cfg, array $cities): string {
    ...
    return str_replace(
        ['{player}', '{channel}', '{city}', '{game}', '{car}', '{track}', '{series}'],
        [$names[array_rand($names)], $cfg['channelName'], $cities[array_rand($cities)], $cfg['game'] ?? '', $cfg['car'] ?? '', $cfg['track'] ?? '', $cfg['series'] ?? ''],
        $text
    );
}
...
// ACC streams use 'aviation' slot for game-specific messages
